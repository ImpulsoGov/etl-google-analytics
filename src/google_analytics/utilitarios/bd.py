...
def postgresql_copiar_dados(
    ...
    with conector_dbapi.cursor() as cursor:  # type: ignore
        ...
        enumeracao_colunas = ", ".join('"{}"'.format(coluna) for coluna in colunas)
...
def carregar_dataframe(
    ...
    logger.info(
        "Carregando {num_registros} registros " "para a tabela `{tabela_destino}`...",
    ...
    df[colunas_data] = df[colunas_data].applymap(lambda dt: dt.isoformat() if pd.notna(dt) else None)
